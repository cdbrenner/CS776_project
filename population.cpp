...
void Population::evaluate(int choice, int data[][48], int random_seed, int srand_offset)
{
    for(int i = 0; i < options.population_size; i++)
    {
        try
        {
            eval(members[i], m_results, data, choice, random_seed, srand_offset + i);
        }
...
void Population::evaluate_single(Individual& member, int choice, int data[][48], int random_seed, int srand_offset)
{
    for(int i = 0; i < options.population_size; i++)
    {
        try
        {
            eval(member, m_results, data, choice, random_seed, srand_offset + i);
        }
        catch(std::string error_message)
            {throw error_message;}
...
void Population::genitor(int srand_offset)
{
    // TEST
    ...
    // REAL
    TSP(child, options.tsp_data, -1, m_results);
...
void Population::sort_member_ids_by_fitness(int random_seed, int srand_offset)
{
...
void Population::rank_selection_prep(int srand_offset)
{
    set_member_ids();
...
void Population::edge_recombination(Individual* parent_1, Individual* parent_2, Individual& child, int srand_offset)
{
...
void Population::scramble_mutation(Individual& member, int srand_offset)
{
