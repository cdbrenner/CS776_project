...
GA::GA(int argc, char *argv[], int iterator, std::string problem_type, int ga_variant_option, std::string ga_variant_name_abbreviation, double mutation_rate, double xover_rate, int chromosome_length, std::ofstream& log_stream)
{
    for(int i = 0; i < 48; i++)
        for(int j = 0; j < 48; j++)
...
void GA::init(int eval_option, int report_option)
{
    ...
    try
    {
        parent->evaluate(eval_option, options.tsp_data, options.random_seed, 0);
    }
...
void GA::run(int eval_option, int report_option)
{
    ...
    for(int i = 1; i < options.max_generations; i++)
    {
        ...
        try
        {
            child->evaluate(eval_option, options.tsp_data, options.random_seed, srand_offset);
        }
        catch(std::string error_message)
        {
            throw(error_message);
...
bool GA::extinction_event(int eval_option, int random_seed, int srand_offset)
{
    ...
    try
    {
        parent->evaluate(eval_option, options.tsp_data, options.random_seed, srand_offset);
    }
...
void GA::set_tsp_data_option(std::string filename)
{
    ...
    if(in.good())
    {
        ...
        {
            case 42: // EDGE_WEIGHT_TYPE: EXPLICIT; EDGE_WEIGHT_FORMAT: LOWER_DIAG_ROW
...
int GA::get_tsp_specs(std::ifstream& in)
{
    if(in.good())
    {
        ...
        while(temp_1 != "EDGE_WEIGHT_FORMAT:")
            in >> temp_1;
        in >> temp_1;
